...
# 이미지 예외 처리
if flag == 2:
  img = imutils.resize(img_err, width=690)  # 650 # 688
  #cv2_imshow(img)
  img_copy = img.copy()
  big_contours = pre_img_plate(img)
  rect, contour_list, flag = rect_detect(big_contours)
...
img_roi = img.copy()
roi = img_roi[ymin:ymax, xmin:xmax]
...
src = number_plate.copy()
src2 = src.copy()
src3 = src2.copy()
ex1_img = src.copy()
inner = best_rect(sorted_contourboxes)  # 가장 큰 contour외부는 빼줌
contour_boxes1, src = inner_draw(inner, src)

all_area = src.shape[1] * src.shape[0]
